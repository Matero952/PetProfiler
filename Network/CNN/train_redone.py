...
def penis_t(model_path, test_path):
    torch.cuda.empty_cache()
    test_images = get_image_files(test_path)
    print(f"Test images: {test_images}")
    dls = ImageDataLoaders.from_path_func(test_path, test_images, label_func= label_func, valid_pct=0.95, bs=1)
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap="PiYG", cbar=True,
                xticklabels=['Dog Class', 'Null Class'], yticklabels=['Dog Class', 'Null Class'], annot_kws={"size": 14} )
    plt.title("Fall Environment-Based Testing Confusion Matrix", size=14, pad=15, fontweight="bold")
    plt.xlabel('Predicted Label', size= 13, fontweight='bold')
    plt.ylabel('True Label', size= 13, fontweight='bold')
    plt.tight_layout()
    plt.show()

    return preds, targets

penis_t('/home/mateo/Github/PetProfiler/model_v4.pkl', '/home/mateo/datasets/raw_dataset/fall')